...
<xsl:choose>
    ...
    <xsl:if test="count(cml:open_state) =1 and count(cml:closed_state) =1"> <!-- Classical HH -->
        ...
        <xsl:if  test="name()!='conc_dependence'">
            <xsl:choose>
                <xsl:when  test="@expr_form != 'generic' and @expr_form != 'tabulated' ">
    ? Found a parameterised form of rate equation for <xsl:value-of select="@name"/>, using expression: <xsl:choose>
   ...
   <xsl:value-of select="$A_name"/> = <xsl:value-of select="$A_name"/> * <xsl:call-template name="convert">
    ...
    <xsl:value-of select="@name"/> = <xsl:value-of select="$A_name"/> * vtrap((v - <xsl:value-of select="$Vhalf_name"/>), <xsl:value-of select="$B_name"/>)<xsl:text>
                ...
                <xsl:when test="@expr_form = 'generic' or @expr_form = 'tabulated'">
                    <xsl:variable name="expr">
                        <xsl:choose>
                            <xsl:when test="@expr_form = 'generic'"><xsl:value-of select="@expr" /></xsl:when>
                            <xsl:when test="@expr_form = 'tabulated'">table_<xsl:value-of select="@name"/>_<xsl:value-of select="$gateName"/>(v)</xsl:when>
                        </xsl:choose>
                    </xsl:variable>
    ? Found a generic/tabulated form of the rate equation for <xsl:value-of select="@name"/>, using expression: <xsl:value-of select="$expr" /><xsl:text>
    </xsl:text>  
                    <xsl:if test="string($xmlFileUnitSystem) = 'SI Units'">
    ? Note: Equation (and all ChannelML file values) in <xsl:value-of select="$xmlFileUnitSystem"/> so need to convert v first...<xsl:text>
    </xsl:text>
...
<xsl:for-each select="cml:current_voltage_relation/cml:gate/cml:transition/cml:table">

    <xsl:variable name="gateName"><xsl:value-of select="../../@name"/></xsl:variable>
    <xsl:variable name="transitionName"><xsl:value-of select="../@name"/></xsl:variable>
    ?     *** Adding a table function for <xsl:value-of select="$transitionName"/> in <xsl:value-of select="$gateName"/> ***

FUNCTION table_<xsl:value-of select="$transitionName"/>_<xsl:value-of select="$gateName"/>(v) {
    LOCAL tableValue, numTableEntries, prevEntry, vMin, vMax, fract

    numTableEntries = <xsl:value-of select="count(cml:entry)"/>
    vMin = <xsl:value-of select="@xmin"/>
    vMax = <xsl:value-of select="@xmax"/>

    ? A table is present in the ChannelML file for this function with <xsl:value-of select="count(cml:entry)"/> entries from <xsl:value-of select="@xmin"/> to <xsl:value-of select="@xmax"/>
    
    if (v &lt;= vMin) { tableValue = 0 }
    else if (v >= vMax) { tableValue = numTableEntries }
    else { tableValue = (numTableEntries-1) * (v-vMin)/(vMax-vMin) }

    
    
<xsl:for-each select="cml:entry">
    ? entry val <xsl:value-of select="position()"/> = <xsl:value-of select="@value"/>
    
    <xsl:if test="position() > 1">
 
    if (tableValue >= <xsl:value-of select="position() - 2"/> &amp;&amp; tableValue &lt; <xsl:value-of select="position() -1"/>) {
        fract = (tableValue - <xsl:value-of select="position() -2"/>)
        table_<xsl:value-of select="$transitionName"/>_<xsl:value-of select="$gateName"/> = prevEntry + fract*(<xsl:value-of select="@value"/>-prevEntry)
    }
    </xsl:if>
    prevEntry = <xsl:value-of select="@value"/>
    
</xsl:for-each>

printf(".. Evaluated function table_<xsl:value-of select="$transitionName"/>_<xsl:value-of select="$gateName"/> at v: %g (tableValue: %g, fract: %g): %g\n", v, tableValue, fract, table_<xsl:value-of select="$transitionName"/>_<xsl:value-of select="$gateName"/>)
