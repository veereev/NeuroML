...
function make_<xsl:value-of select="@name"/>
        ...
        */
        ...
        create leakage {chanpath}
            ...
            <xsl:when test="count(cml:current_voltage_relation/cml:ohmic/cml:conductance/cml:gate/cml:state/cml:transition/cml:voltage_conc_gate) &gt; 0">
        ...
        float dv = ({v_max} - {v_min})/{tab_divs}
            ...
            <xsl:choose>      
                <xsl:when test="count(cml:state/cml:transition/cml:voltage_conc_gate) &gt; 0">
        ...
        float c
        float conc_min = <xsl:value-of select="@min_conc"/>
        float conc_max = <xsl:value-of select="@max_conc"/>
        ...
        for (i = 0; i &lt;= ({tab_divs}); i = i + 1)
            ...
            </xsl:for-each> <!-- <xsl:for-each select='cml:state/cml:transition/*/* ... etc>-->
            
            <xsl:variable name='tableEntry'>
                <xsl:choose>
                    <xsl:when test="count(cml:state/cml:transition/cml:voltage_gate) &gt; 0">table[{i}]</xsl:when>
                    <xsl:when test="count(cml:state/cml:transition/cml:voltage_conc_gate) &gt; 0">table[{i}][{c}]</xsl:when>
                </xsl:choose>
            </xsl:variable>
                           
            <!-- Working out the conversion of alpha and beta to tau & inf-->
            <xsl:choose>
                <xsl:when test="count(cml:state/cml:transition/cml:voltage_gate/cml:alpha | cml:state/cml:transition/cml:voltage_conc_gate/cml:alpha)=1 and
                                count(cml:state/cml:transition/cml:voltage_gate/cml:beta | cml:state/cml:transition/cml:voltage_conc_gate/cml:beta)=1 and
                                count(cml:state/cml:transition/cml:voltage_gate/cml:tau | cml:state/cml:transition/cml:voltage_conc_gate/cml:tau)=0 and
                                count(cml:state/cml:transition/cml:voltage_gate/cml:inf | cml:state/cml:transition/cml:voltage_conc_gate/cml:inf)=0">
            ...
            <xsl:if test="$consoleOutput='yes'">echo "Tab <xsl:value-of select="$gateRef"/>: v: "{v} ", a: "{alpha} ", b: "{beta} ", tau: "{tau} 
                <xsl:if test="count(cml:state/cml:transition/cml:voltage_conc_gate) &gt; 0">
            echo "Tab <xsl:value-of select="$gateRef"/>: conc: " {<xsl:value-of select="cml:state/cml:transition/cml:voltage_conc_gate/cml:conc_dependence/@variable_name"/>}
                </xsl:if>
            </xsl:if>            
            ...
            // Using the tau and inf expressions to populate the tables
                    ...
                    <xsl:choose>
                        <xsl:when test="count(cml:state/cml:transition/cml:voltage_gate/cml:tau | cml:state/cml:transition/cml:voltage_conc_gate/cml:tau)=0">
            float tau = 1/(temp_adj * (alpha + beta))   
                        </xsl:when>
                        <xsl:otherwise>
            tau = tau/temp_adj
                    ...
                    <xsl:if test="count(cml:state/cml:transition/cml:voltage_gate/cml:inf | cml:state/cml:transition/cml:voltage_conc_gate/cml:inf)=0">
            ...
            v = v + dv
                ...
                <xsl:if test="count(cml:state/cml:transition/cml:voltage_conc_gate) &gt; 0">
            <xsl:value-of select="cml:state/cml:transition/cml:voltage_conc_gate/cml:conc_dependence/@variable_name"/> = <xsl:value-of 
                select="cml:state/cml:transition/cml:voltage_conc_gate/cml:conc_dependence/@variable_name"/> + dc
        end // end of for (c = 0; c &lt;= ({tab_divs}); c = c + 1) 
                </xsl:if>   
             
            <xsl:if test='count(cml:state/cml:transition/cml:voltage_gate/cml:tau | cml:state/cml:transition/cml:voltage_conc_gate/cml:tau) &gt; 0 or
                      count(cml:state/cml:transition/cml:voltage_gate/cml:inf | cml:state/cml:transition/cml:voltage_conc_gate/cml:inf) &gt; 0'>
