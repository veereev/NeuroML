...
<!--
    ...
    This file is used to convert v1.7 NeuroML files (morphology and/or network structure)
...
<xsl:template match="net:networkml">
    ...
    <xsl:for-each select="net:projections/net:projection">
        <xsl:variable name="src"><xsl:value-of select="net:source"/></xsl:variable>
        <xsl:variable name="tgt"><xsl:value-of select="net:target"/></xsl:variable>
        ...
        <xsl:for-each select="net:connections/net:connection">
            <xsl:variable name="preCellId"><xsl:value-of select="net:pre/@cell_id"/></xsl:variable>
            <xsl:variable name="postCellId"><xsl:value-of select="net:post/@cell_id"/></xsl:variable>
