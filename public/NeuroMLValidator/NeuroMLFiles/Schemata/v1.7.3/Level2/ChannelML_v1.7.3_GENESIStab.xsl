...
<xsl:text>// This is a GENESIS script file generated from a ChannelML v1.7.3 file
        ...
        <xsl:if test="$postV1_7_3format = 'yes'">          
            ...
            float A, B, Vhalf
                        <xsl:call-template name="generateNewEquation">
                            ...
                            <xsl:with-param name="A_cml" select="@rate"/>
                            <xsl:with-param name="B_cml" select="@scale"/>
                            <xsl:with-param name="Vhalf_cml" select="@midpoint"/>
                        </xsl:call-template>
                    </xsl:when>
...
<xsl:template name="generateNewEquation">
    ...
    <xsl:param name="functionForm" />
    <xsl:param name="A_cml" />
    <xsl:param name="B_cml" />
    <xsl:param name="Vhalf_cml" />
    <xsl:choose>

        <xsl:when test="string-length($functionForm) &gt; 0"> <!-- So not an empty string-->
            // ChannelML form of equation: <xsl:value-of select="$name"/> which is of form <xsl:value-of select="$functionForm" />, with params:
            // A = <xsl:value-of select="$A_cml"/>, B = <xsl:value-of select="$B_cml" />, Vhalf = <xsl:value-of
            select="$Vhalf_cml" />, in units: <xsl:value-of select="$xmlFileUnitSystem"/>
            ...
            B = <xsl:call-template name="convert">
                    <xsl:with-param name="value"><xsl:value-of select="$B_cml"/></xsl:with-param>
                    <xsl:with-param name="quantity">Voltage</xsl:with-param>
                </xsl:call-template>
            Vhalf = <xsl:call-template name="convert">
                    <xsl:with-param name="value"><xsl:value-of select="$Vhalf_cml"/></xsl:with-param>
                    <xsl:with-param name="quantity">Voltage</xsl:with-param>
                </xsl:call-template><xsl:text>
            </xsl:text>
