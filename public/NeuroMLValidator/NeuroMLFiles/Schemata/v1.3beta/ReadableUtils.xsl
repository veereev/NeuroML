    ...
    <xsl:choose> 
        <xsl:when test="$xmlFileUnitSystem  = 'Physiological Units'">
            <xsl:choose>
                ...
                <xsl:when test="$quantity = 'Concentration'"> mM?</xsl:when>
...
<xsl:template name="formatExpression">
    ...
    When <xsl:value-of select="substring-before($oldExpression,'?')"/>: <xsl:text>
    </xsl:text><xsl:value-of select="$variable"/>(v) = <xsl:value-of select="$ifTrue"/>, otherwise: <xsl:text>
    
    </xsl:text><xsl:value-of select="$variable"/>(v) = <xsl:value-of select="$ifFalse"/>
    
        </xsl:when>
        <xsl:otherwise>
    <xsl:value-of select="$variable"/> = <xsl:value-of select="$oldExpression"/>
        </xsl:otherwise>
    </xsl:choose>
</xsl:template>
...
<xsl:template match="meta:authorList">
        <xsl:call-template name="tableRow">
                ...
                <xsl:with-param name="value"><xsl:for-each select="meta:modelAuthor">&lt;b&gt;<xsl:value-of select="."/>&lt;/b&gt;</xsl:for-each>
                </xsl:with-param>
        </xsl:call-template>
</xsl:template>
